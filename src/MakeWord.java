...
public class MakeWord {
	...
	private boolean HasTextImg(String text){
		...
		if(text.length()>this.indicatePic.length()){	//compare with picture indicate character
			...
			IsPicture=PicFind.compareTo(this.indicatePic); 	//IsPicture == 0 占쎈���域밸챶�앾옙占쏙옙占쏙옙占쏙옙占쏙옙占�
		...
		for(j=0; j<problemBody.size(); j++){
			if(j==0)
				...
				for(int i=0; i<item.size()-1; i++)	//last memember is problem ID
				{
					...
					switch(i){
					case 0:
						this.writeText(proNum.toString() + ". " + remain, align);
						break;
					case 1:{
						if(imgIn){
					...
					case 2:{
						if(imgIn){
							WriteTextWithImg("가. "+remain, align);
						}
						else
							this.writeText("가. "+remain, align);
						break;
					}
					case 3:{
						if(imgIn){
							WriteTextWithImg("나. "+remain, align);
						}
						else
							this.writeText("나. "+remain, align);
						break;
					}
					case 4:{
						if(imgIn){
							WriteTextWithImg("다. "+remain, align);
						}
						else
							this.writeText("다. "+remain, align);
						break;
					}
					case 5:{
						if(imgIn){
							WriteTextWithImg("라. "+remain, align);
						}
						else
							this.writeText("라. "+remain, align);
						break;
					}
					case 6:{
