...
class Query {
	...
	Query(){ //DB Open
		...
		try{
            ...
            msg.alertMessage(null, "JDBC 드라이버가 존재하지 않습니다. "+e, "Query");
        }
		catch(Exception e){
			 msg.alertMessage(null, "데이터베이스 접속 실패. "+e, "Query");
		}	
		try {
...
//		Vector<String> result = new Vector<String>();
	...
	public int doInserts(String into, String values) {
		...
		String q;
		 ...
		 try {
			...
			msg.alertMessage(null, "insert 성공", "query/doInsert");
			return 0;
		} catch (SQLException e) {
			 msg.alertMessage(null, "insert 실패", "query/doInsert");
			 return -1;
		}	
	}
	...
	public int doUpdates(String table, Vector<String> set, String where) {
		...
		try {
			System.out.println(q);
			statement.executeUpdate(q);
			msg.alertMessage(null, "업데이트 성공", "query/doUpdates");
			return 0;
		} catch (SQLException e) {
			 msg.alertMessage(null, "업데이트 실패", "query/doUpdates");
			 return -1;
		}		 
	...
	public int doDeletes(String from, String where) {
		...
		String q;
		 ...
		 try {
			 System.out.println(q);
			 statement.executeUpdate(q);
			 msg.alertMessage(null, "삭제 성공", "query/doDeletes");
			 return 0;
		 } catch (SQLException e) {
			 msg.alertMessage(null, "삭제 실패", "query/doDeletes");
			 return -1;
		 }	
	}
