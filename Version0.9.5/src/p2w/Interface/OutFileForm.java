...
public class OutFileForm extends JFrame {
	...
	private class ProblemListManager {
		...
		public void insertToList() //문제 추가 버튼을 누를시 해당하는 문제들을 저장
		{
			...
			if(error_count>0)
				message.alertMessage(null, error_count + "개의 이미 추가된 문제와 중복된 문제 있습니다. 원하는 문제 수보다 적게 추가 될 수 있습니다.");
		}
	...
	private class ButtonClickListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			else if(button.getName() == "print_Btn") //출력 버튼 클릭시
			{
				...
				if(choice==JFileChooser.APPROVE_OPTION){
					 ...
					 */
					try {	//MakeWord file
						file = new MakeWord(problemHeader, problemBody, path.toString()+".docx");
						...
						msg.alertMessage(plist, "오류가 발생하였습니다.");
						e1.printStackTrace();
					}
					
					try { //그림의 경로가 다른 경우 예외 처리
						...
						msg.alertMessage(plist, e1.toString().substring(31));
						e1.printStackTrace();
					}
					...
					if(!isErr){
						...
						MessageCall msg = new MessageCall();
						msg.alertMessage(plist, "작업을 완료하지 못하였습니다.");
					}
				}
			}
		...
		private String makeProblemInfo(int problemID){
			...
			if(problemHeader.size()>1){
				...
				for(int i=0;i<(problemBody.get(1).size());i++)
