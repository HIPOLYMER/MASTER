...
import p2w.DataBase.GetLMS;
import p2w.DataBase.Query;
...
class SelectOptionsInsForm extends JFrame {
	...
	SelectOptionsInsForm() {
		...
		//label
		year_L.setForeground(new Color(217, 0, 35));
		year_L.setBorder(new EmptyBorder(10,10,0,10));
		
		serial_L.setForeground(new Color(217, 0, 35));
		serial_L.setBorder(new EmptyBorder(10,10,0,10));
		
		type_L.setForeground(new Color(217, 0, 35));
		type_L.setBorder(new EmptyBorder(10,10,0,10));
		
		subject_L.setForeground(new Color(217, 0, 35));
	...
	private class ButtonClickListener implements ActionListener, ErrCheck {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (button.getName() == "insert_Btn") {
				...
				if(checkEmpty()==false){
				...
				}else if(isNum(pNum_TB.getText())==false){
					message.alertMessage(button.getRootPane().getParent(), "문제번호는 숫자만 입력 가능합니다.");
				}else{
					...
					if(check == 0){ //haeder insert성공시에만
						query.doInserts(pbTable, insert_pbTable);
						message.alertMessage(button.getRootPane().getParent(), "문제 입력 성공");
						autoNum++;
					}else{
						message.alertMessage(button.getRootPane().getParent(), "같은 번호의 문제가 존재합니다.");
					}
					query.close();
	...
	//				|| (addition_TA.getText().isEmpty())
				...
				return false;
			}else if(explainF_TA.getText().isEmpty()){// 풀이 는 입력이 안 됐다면 풀이를 입력하지 않을 것인지 물어보는 메시지 상자를 띄우고, 예라면 그냥 넘어가고
	...
	private class InsertProblemForm extends JFrame {
		...
		InsertProblemForm() {
			...
			Border numBorder = new TitledBorder(null, "번호", TitledBorder.LEADING, TitledBorder.ABOVE_TOP, null, new Color(217, 0, 35));
			pNum_TB.setBorder(numBorder);
			...
			problem_TA.setBorder(new TitledBorder(null, "문제", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(217, 0, 35)));
			...
			example_P.setBorder(new TitledBorder(null, "보기", TitledBorder.LEADING, TitledBorder.ABOVE_TOP, null, new Color(217, 0, 35)));
			...
			explainF_TA.setBorder(new TitledBorder(null, "Full", TitledBorder.LEADING, TitledBorder.TOP, null,new Color(217, 0, 35)));
			...
			solexam_P.setBorder(new TitledBorder(null, "답", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(217, 0, 35)));
