...
public class MakeWord {
	...
	private static Vector<String> keySolution;
	private static Vector<String> fullSolution;
	...
	private boolean HasTextImg(String text){
		...
		if(text.length()>this.indicatePic.length()){	//compare with picture indicate character
			...
			IsPicture=PicFind.compareTo(this.indicatePic); 	//IsPicture == 0 占쎌��ε����������紐�덧���鍮�옙�용�占쎌���옙醫��占쎌���옙���占쎌�猷�옙�용���옙
	...
	private void WriteTextWithImg(String content, char align, String size, boolean isbold) throws Exception{
	...
	//7. write body
	public void writeProblem(boolean keyword, boolean solution, char align) throws Exception{
		int j=0;
	
		if(keyword){	//keyword is checked
			keySolution=new Vector<String>();
		}
		if(solution){	//solution is checked
			fullSolution=new Vector<String>();
		}
		...
		for(j=0; j<problemBody.size(); j++){
			...
			if(j==problemBody.size()-1){	//last problem we didn't want blank in the paper.
				this.PageBreak();
				//��� 異��
				this.writeText("정답 출력", 'b', "18", true);
				this.writeText(pro_ans, 'b', "18", false);
				//��� 異��
				
				if(solution || keyword){
					this.PageBreak();
					this.writeText("풀이 출력", 'b', "18", true);
				}
				
				if(keyword&&solution){	//keyword and solution both are checked.
					writeSolution(keySolution, fullSolution, problemBody.size()-1);
				}
				else{
					if(keyword){	//only keyword is checked.
						writeSolution(keySolution, " keyword 풀이");
					}
					if(solution){	//only solution is checked.
						writeSolution(fullSolution, " full 풀이");
					}
				}				
			}
			else
				this.writeBr(2);	//one problem is printed. To separate from previous one.
		}

	}
	
	// 異�� 
	private void writeSolution(Vector<String> con, String add) throws Exception{
		
		int numOfProblem=0;
		
		for(String textInput : con){
			
			boolean imgIn=HasTextImg(textInput);
			String remain;
			
			numOfProblem++;
			
			if(imgIn){
				remain=textInput.substring(this.indicatePic.length());
			}
			else{
				remain=new String(textInput);
			}
			
			this.writeText(Integer.toString(numOfProblem) + add, 'b', "18", false);
			
			if(imgIn){
				WriteTextWithImg(remain, 'b', "18", false);
			}
			else
				this.writeText(remain, 'b', "18", false);
		}
	}

	// 異��
	private void writeSolution(Vector<String> key, Vector<String> full, int maxNum) throws Exception{
		
		int numOfProblem=0;
		int num=1;
		
		for(;numOfProblem<maxNum; numOfProblem++){
			String keyText=key.get(numOfProblem);
			String fullText=full.get(numOfProblem);
			
			//below for keyword
			boolean imgInKey=HasTextImg(keyText);
			String remainKey;
			
			if(imgInKey){
				remainKey=keyText.substring(this.indicatePic.length());
			}
			else{
				remainKey=new String(keyText);
			}
			
			this.writeText(Integer.toString(num) + " keyword 풀이", 'b', "18", false);
			
			if(imgInKey){
				WriteTextWithImg(remainKey, 'b', "18", false);
			}
			else
				this.writeText(remainKey, 'b', "18", false);
			
			//this part for solution
			boolean imgInFull=HasTextImg(fullText);
			String remainFull;
			
			if(imgInFull){
				remainFull=fullText.substring(this.indicatePic.length());
			}
			else{
				remainFull=new String(fullText);
			}
			
			this.writeText(Integer.toString(num) + " full 풀이", 'b', "18", false);
			
			if(imgInFull){
				WriteTextWithImg(remainFull, 'b', "18", false);
			}
			else
				this.writeText(remainFull, 'b', "18", false);
			
			num++;//counting the number of problems.
		}
	}
