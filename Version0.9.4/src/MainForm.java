	...
	private class ButtonClickListener implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent e) {
			...
			else if(evt_Btn.getName()=="dbClear_Btn")
			{
				...
				int check1=0;
				int check2=0;
				check1 = message.yesnoMessage(null, "정말 초기화 하시겠습니까?");
				if(check1 == JOptionPane.YES_OPTION){
					check2 = message.yesnoMessage(null, "모든 내용이 지워집니다.");
			...
			else if(evt_Btn.getName()=="add_Btn")
			{
				...
				if(!year_TB.getText().isEmpty()){
					...
					if(check == 0){
						...
						message.alertMessage(null, "새로운 년도가 추가되었습니다.");
					}else{
						message.alertMessage(null, year_TB.getText().toString()+"년도 추가에 실패하였습니다.");
					}
					...
					if(!large_TB.getText().isEmpty()){
						if(!medium_TB.getText().isEmpty()){
							if(!small_TB.getText().isEmpty()){
								...
								message.alertMessage(null, "과목,대분류,중분류,소분류 추가 완료");
							}else{
								query.doInserts("classification","null,\""+subject_TB.getText().toString()+"\""
										+",\""+large_TB.getText().toString()+"\""
										+",\""+medium_TB.getText().toString()+"\""
										+",\"null\"");
								message.alertMessage(null, "과목,대분류,중분류 추가 완료");
							}
						}else{
							query.doInserts("classification","null,\""+subject_TB.getText().toString()+"\""
									+",\""+large_TB.getText().toString()+"\""
									+",\"null\""
									+",\"null\"");
							message.alertMessage(null, "과목,대분류 추가 완료");
						}
					}else{
						query.doInserts("classification","null,\""+subject_TB.getText().toString()+"\""
								+",\"null\""
								+",\"null\""
								+",\"null\"");
						message.alertMessage(null, "과목 추가 완료");
					}
					if(!year_TB.getText().isEmpty() && !subject_TB.getText().isEmpty()){
						message.alertMessage(null, "입력된 항목이 없습니다.");
					}
